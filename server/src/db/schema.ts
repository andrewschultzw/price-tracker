...
export function initializeSchema(): void {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS trackers (
      ...
      -- Buy-arm feature (migration v19). buy_armed=1 enables autonomous
      -- purchasing when the price hits threshold. buy_quantity controls how
    ...
    -- Buy-arm purchase intents (migration v19). One row per arm event;
    -- tracks the lifecycle from 'armed' through approval, execution, and
    -- resolution. token is a short-lived secret used by the confirmation
    -- webhook so only the intended action can trigger a purchase.
    ...
    CREATE INDEX IF NOT EXISTS idx_purchase_intents_tracker_id ON purchase_intents(tracker_id);
