...
- [x] **Browser extension (the extension half of the third big bet).** ~~Chrome MV3 extension, sideload-only for v1, one-click "Track this" from any retailer page via toolbar icon + right-click context menu.~~ **Done 2026-05-06:** new `extension/` workspace (Vite + @crxjs/vite-plugin + TypeScript MV3) with toolbar icon + right-click context menu, popup confirmation form, "Already tracking" state on revisit (60s `chrome.storage.session` cache), loading spinner, options page for token paste-in. Server-side per-user API tokens feature (Settings → Connected Apps): migration v12 (`user_api_tokens` table), SHA-256 hashed at rest with soft-delete, plaintext shown once. `apiKeyMiddleware` extended to accept user-issued tokens alongside the existing global env var. Server tests 488 → 511 (+23). Client tests 76 → 80 (+ component test infra: testing-library + jsdom). New extension test suite at 7 tests (message guards + normalize-url parity with server). Spec: `docs/superpowers/specs/2026-05-06-browser-extension-design.md`. Plan: `docs/superpowers/plans/2026-05-06-browser-extension.md`. [PR #15](https://github.com/andrewschultzw/price-tracker/pull/15).
  ...
  **v1 fast-follows from final review (sub-threshold but worth tracking):**
  - [ ] **`appendToCache` race window.** Two popups simultaneously calling CREATE could lose one cache append; second-added tracker invisible to CHECK_DUP for the 60s TTL window. Worst case: a redundant tracker created. Fix: lock around read-modify-write, or refetch cache server-side after CREATE. `extension/src/background/service-worker.ts:48-55`.
  - [ ] **No drift detection on `extension/src/types/api.ts`.** Hand-mirror of server's `Tracker` shape with no automated parity test (compare to `normalize-url.ts` which has one). If the server adds/renames a field on `Tracker`, the extension compiles fine but silently misses it. Fix options: types-only npm workspace package, or a contract test against a running server.
  - [ ] **VALIDATION errors leak raw zod JSON to popup UI.** A 400 from `POST /api/trackers` lands in the popup as `URL doesn't look right ({"fieldErrors":{"url":["Invalid URL"]}}).` Functional but ugly. Fix: extract a single human field from the flatten, or suppress `detail` for VALIDATION. `extension/src/lib/api.ts:47` + `extension/src/popup/popup.ts:131`.
  - [ ] **`errorText(code: string)` should be `errorText(code: ErrorCode)`.** Loses exhaustiveness checks against the `ErrorCode` discriminated union — a new code added to `messages.ts` won't fail compilation in the popup error renderer. Fix: tighten parameter type and add a `const _exhaustive: never = code;` default. `extension/src/popup/popup.ts:125`.
  - [ ] **Hostname display can show non-hostname strings.** `new URL('chrome://settings').hostname` returns `'settings'`. User could even submit such a URL via the form (server-side `normalizeTrackerUrl` will return null and the entry will scrape-fail, but UX is rough). Fix: validate hostname shape against `^([a-z0-9-]+\.)+[a-z]{2,}$` early and show "Cannot track this page." `extension/src/popup/popup.ts:38`.
  - [ ] **Spec drift: global key path kept plaintext compare instead of hash-compare.** Spec's "Middleware extension" section line 75 says the global `PRICE_TRACKER_API_KEY` should also be hash-compared "like user tokens." Implementation kept the existing `timingSafeEqual` over plaintext (functionally equivalent for security — `timingSafeEqual` is constant-time on plaintext too). Either update the spec to match reality or migrate the global key path to hash-compare.
