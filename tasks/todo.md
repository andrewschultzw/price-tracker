...
- [ ] **Autonomous purchasing (buy-on-trigger).** Set a "spend" budget for a bucket, a single tracker, or a group of trackers, and let Claude execute the purchase automatically when a price hits the buy limit — with hard spend caps and full safety controls. **In design** — brainstorming 2026-05-29; spec to land in `docs/superpowers/specs/`. This is the riskiest feature in the portfolio (real money, real account, real liability), so design gets extra rigor before any code.
